...
pub fn run() {
    ...
    let ml_config = aleym_core::ml::scheduler::Config {
        ...
        publication_window_new_items_count_threshold: config
            .scheduler
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let state = app.manage(State { repr });
            // scheduler thread
            tauri::async_runtime::spawn(async move {
                sched_repr.start_scheduler(notif, ml_config).await.unwrap()
            });
            ...
            tauri::async_runtime::spawn(async move {
                ...
                proxy_repr
                    .network
                    .run_tor_socks5_proxy(proxy_addr)
                    .await
                    .unwrap();
            });
